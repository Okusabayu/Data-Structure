...
void insertAfter(struct Node* prev_node, int new_data)
{
    ...
    new_node->prev = prev_node; // Set the previous of the new node to the previous node

    /* 7. Change previous of new_node's next node */
    if (new_node->next != NULL) // If the next node of the new node exists
        new_node->next->prev = new_node; // Set the previous of the next node of the new node to the new node
}
